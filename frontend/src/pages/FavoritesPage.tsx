...
function FavoritesPage() {
  ...
  if (favorites.length === 0) {
    return (
      <main className="flex flex-1 flex-col items-center justify-center gap-3 px-6 py-24 text-center">
        ...
        <h1 className="text-2xl font-semibold text-neutral-900">{t('favorites.noFavorites')}</h1>
        <p className="max-w-md text-neutral-500">{t('favorites.noFavoritesSubtitle')}</p>
      </main>
    )
  }
  ...
  return (
    <main className="mx-auto max-w-7xl flex-1 px-4 py-10 sm:px-6 lg:px-8">
      <h1 className="mb-8 text-2xl font-semibold text-neutral-900">{t('favorites.title')}</h1>

      {destinationFavorites.length > 0 && (
        <section className="mb-10">
          <h2 className="mb-4 text-lg font-semibold text-neutral-900">{t('favorites.destinations')}</h2>
      ...
      {placeFavorites.length > 0 && (
        <section>
          <h2 className="mb-4 text-lg font-semibold text-neutral-900">{t('favorites.places')}</h2>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
